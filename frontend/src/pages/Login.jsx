                ...
                <div className="w-full max-w-md space-y-8">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <Button type="submit" disabled={isSubmitting} className="w-full h-12 rounded-xl bg-primary text-primary-foreground gap-2">
                            {isSubmitting ? <Loader2 className="w-4 h-4 animate-spin" /> : <><span>{t("Auth.SignInBtn")}</span><ArrowRight className="w-4 h-4" /></>}
                        </Button>
                    </form>

                    <div className="text-center">
                        <span className="text-sm text-muted-foreground">{t("Auth.NoAccount")} </span>
                        <Link to="/signup" className="text-sm text-primary hover:underline font-medium">{t("Auth.SignUpLink")}</Link>
                    </div>
                </div>
            </motion.div>
